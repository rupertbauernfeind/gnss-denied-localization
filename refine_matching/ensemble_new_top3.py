#!/usr/bin/env python3
"""
Ensemble the NEW top 3 models: minima-roma, loftr, xoftr

Usage:
    python ensemble_new_top3.py
"""
...
def create_ensemble(predictions, rough_pred, ensemble_func, max_deviation=150.0,
    ...
    for img_id in sorted(all_ids):
        ...
        if model_scores and ensemble_func.__name__ == 'ensemble_weighted_by_accuracy':
...
def main():
    ...
    # NEW Top 3 models based on latest results
    ...
    print("="*80)
    print("NEW TOP 3 ENSEMBLE")
    print("="*80)
    print(f"Models: {', '.join(models)}")
    print(f"Scores: minima-roma={model_scores['minima-roma']:.2f}, "
          f"loftr={model_scores['loftr']:.2f}, xoftr={model_scores['xoftr']:.2f}\n")
    ...
    # Load predictions from new top 3 models
    predictions = {}
    for model in models:
    ...
    strategies = [
        ('newtop3_median_150', 'Median (filter 150px)', ensemble_median, 150),
        ('newtop3_mean_150', 'Mean (filter 150px)', ensemble_mean, 150),
        ('newtop3_weighted_150', 'Weighted by Accuracy (filter 150px)',
         ensemble_weighted_by_accuracy, 150),
        ('newtop3_median_100', 'Median (filter 100px)', ensemble_median, 100),
        ('newtop3_median_200', 'Median (filter 200px)', ensemble_median, 200),
    ]
    ...
    for strategy_key, strategy_name, ensemble_func, max_dev in strategies:
        ...
        if result:
            ...
            print(f"{strategy_name:<45} Score: {score:5.2f}  @5m: {accuracies['5m']:5.2f}%  "
                  f"Med: {eval_stats['median']:5.1f}px  Ens: {stats['used']:3d}")

    # Compare to individual models
    print("\n" + "="*80)
    print("COMPARISON TO INDIVIDUALS")
    print("="*80)
    ...
    for model in models:
        ...
        if result:
            ...
            print(f"{model:<20} Score: {score:5.2f}  @5m: {accuracies['5m']:5.2f}%  "
                  f"Med: {stats['median']:5.1f}px")
    ...
    # Compare to best individual (minima-roma)
    best_individual = model_scores['minima-roma']
    improvement = best['score'] - best_individual

    if improvement > 0:
        print(f"\n🎉 Ensemble IMPROVES by +{improvement:.2f} over minima-roma!")
    elif improvement < 0:
        print(f"\n⚠️  Ensemble is {-improvement:.2f} worse than minima-roma")
    else:
        print("\nEnsemble matches minima-roma")
