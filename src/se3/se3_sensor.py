...
from se3.se3_dataset import SE3Dataset, SE3FrameMeta
...
class SE3Sensor:
    ...
    Args:
        ...
        cache_images_in_memory: If ``True``, preload all unique frame images into
            RAM and reuse them during iteration.
    """
    ...
    def __init__(
        ...
        self._image_cache_by_frame_id: Dict[int, np.ndarray] = {}
    ...
    def _build_image_cache(self) -> None:
        """Load all unique frame images into memory."""
        for meta in tqdm(self.dataset.frames_by_idx, desc="Building image cache"):
            if meta.frame_id not in self._image_cache_by_frame_id:
                self._image_cache_by_frame_id[meta.frame_id] = self.dataset.load_image_rgb(
                    meta.frame_id
                )
    ...
    def __iter__(self) -> Iterator[SensorSample]:
        ...
        for recording_idx in range(len(self.recordings)):
            ...
            for step_idx, meta in enumerate(seq):
                if self.cache_images_in_memory:
                    img = self._image_cache_by_frame_id[meta.frame_id]
                else:
